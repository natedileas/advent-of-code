...
def path2points(path):
	...
	"""
	path = path.split(',')
	x = np.zeros(len(path) + 1, dtype=np.int16)
	y = np.zeros(len(path) + 1, dtype=np.int16)
	...
	movements = np.vstack([x, y]).T
	points = np.cumsum(movements, axis=0)
	return points


def segmentIntersect(p,r,q,s):
	""" accept column vectors """
	logging.debug('p{}q{}r{}s{}'.format(p,q,r,s))
	numerator = np.cross((q-p), r)
	rxs = np.cross(r, s)
	u = numerator / rxs
	t = np.cross((q-p), s) / rxs
	logging.debug('num: {} denom: {} u: {}'.format(numerator, rxs, u))

	if numerator == 0 and rxs == 0:
		logging.debug('colinear. ')
		return None
	elif numerator != 0 and rxs == 0:
		logging.debug('parallel, non-intersecting')
		return None
	elif rxs != 0 and 0 <= u <= 1 and 0 <= t <= 1:
		logging.debug('intersecting')
		return q + u * s
	else:
		logging.debug('non-intersecting')
		return None


def calcClosestWireCrossing(path1, path2):
	""" """

	# first, convert the paths to boolean arrays
	segments1 = path2points(path1)
	segments2 = path2points(path2)

	logging.debug(segments1)
	logging.debug(segments2)
